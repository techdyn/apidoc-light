...
function createDoc (options) {
  ...
  if (typeof app.options.src === 'string') {
    app.log.debug('Provided "src" option is not an array. Converting it to array.');
  ...
  try {
    ...
    if (api === true) {
      throw new Error('apidoc-light: Nothing to do');
    }

    // Should not happen, `core.parse` error catching is patched
    // if (api === false) {
    ...
    if (!app.options.dryRun) {
      throw new Error('apidoc-light: can only run in `dryRun` mode!');
    }
