...
function getMarkdownParser (options) {
  ...
  const defaultConfig = {
    ...
    wrapper: {
      rules: (ruleFn, options, self) => {
        return (tokens, idx) => {
          return ruleFn(tokens, idx, options, self);
        };
      }
    }
  };
  ...
  if (options.markdown === true) {
    ...
    Object.entries(defaultRules).forEach(([ruleName, ruleFunction]) => {
      markdownParser.renderer.rules[ruleName] =
        defaultConfig.wrapper.rules ?
          defaultConfig.wrapper.rules(ruleFunction, defaultConfig, markdownParser) :
          ruleFunction;
    });
    ...
    if (markdownOptions.rules) {
      Object.entries(markdownOptions.rules).forEach(([ruleName, ruleFunction]) => {
        if (typeof ruleFunction === 'function') {
          markdownParser.renderer.rules[ruleName] =
            userConfig.wrapper.rules ?
              userConfig.wrapper.rules(ruleFunction, userConfig, markdownParser) :
              ruleFunction;
        }
      });
    }
